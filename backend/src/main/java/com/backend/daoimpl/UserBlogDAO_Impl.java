...
import com.backend.dao.UserBlogDAO;
import com.backend.model.UserBlog;

@EnableTransactionManagement
@Repository("userBlogDao")//when we go for dadta base layer we use this 
public class UserBlogDAO_Impl implements UserBlogDAO {
	...
	@Autowired
	private SessionFactory sessionFactory;   //for taransaction of data between the layers we create a session and we perform the actions in this session
   public UserBlogDAO_Impl(){
	...
	public boolean saveUserBlog(UserBlog ubObj) {
		try
		{
			...
			ex.printStackTrace();                      // which will give what type of eception we will get
			return false;
		}
	}
